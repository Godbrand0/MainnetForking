...
async function main() {
    ...
    const approvalUSDT = await USDT.connect(impersonatedSigner).approve(UNIRouter, USDTAmount);
    const tx = await approvalUSDT.wait();
    console.log("USDT approval receipt", tx);

    const approvalUNISWAP = await UNISWAP.connect(impersonatedSigner).approve(UNIRouter, UNISWAPAmount);
    const tx2 = await approvalUNISWAP.wait();
    console.log
    ("UNISWAP approval receipt", tx2);

    const deadline = Math.floor(Date.now()/1000) +60 * 10;

    const provideLiquidity = await Router.connect(impersonatedSigner).addLiquidity(
        USDTAddress, UNISWAPAddress, USDTAmount, UNISWAPAmount, 1, 1, impersonatedSigner.address, deadline
